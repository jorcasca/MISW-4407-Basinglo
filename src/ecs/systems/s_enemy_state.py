...
def system_enemy_state(world: esper.World, player_entity: pygame.Vector2, enemy_spawn: dict, delta_time: float, screen:pygame.Surface):
    ...
    for _, (c_v, c_t, c_a, c_pst, c_t_e) in components:
        ...
        if c_pst.state == EnemyState.CHASE:
            chase_detected = True
            _do_chase_state(c_v, c_t, c_pst, pl_t, enemy_spawn[c_t_e.type]["velocity"], delta_time)
        if c_pst.state == EnemyState.RETURN:
            _do_return_state(c_t, c_pst, enemy_spawn[c_t_e.type]["velocity"], delta_time, screen, current_state)

    if len(components) > 0 and not chase_detected:
        _, (c_v2, _, _, c_pst2, _) = random.choice(components)
        c_v2.vel.x = 0
...
def _do_chase_state(c_v: CVelocity, c_t: CTransform, c_pst: CEnemyState, pl_t: CTransform, velocity: int, delta_time: float):
...
def _do_return_state(c_t: CTransform, c_pst: CEnemyState, velocity: int, delta_time: float, screen:pygame.Surface, stateIfReturn: EnemyState):
    ...
    if direction.magnitude_squared() > 0.5:
            if screen.get_height() - c_t.pos.y < 50:
                ...
                _move_towards_target(c_t, direction, 70, delta_time) 
            else:   
                _move_towards_target(c_t, direction, velocity, delta_time)
    else:
